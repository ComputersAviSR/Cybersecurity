<################################################################################>
# SYNOPSIS                                                                      #
# This script is designed to authenticate and fetch Users from                  #
# BeyondTrust API.                                                              #
#                                                                               #
# DESCRIPTION                                                                   #
# The script uses predefined API key and user credentials to authenticate       #
# against the BeyondTrust API and fetches the list of user.                     #
#                                                                               #
#                                                                               #
# VERSION                                                                       #
# 1.1.1                                                                         #
...
#                                                                               #
# PURPOSE                                                                       #
# To automate the process of fetching user details from BeyondTrust             #
...
function AuthenticateAndFetchUsers {
    ...
    try {
        ...
        $usersResult = Invoke-RestMethod -Uri $uri2 -Method GET -Headers $headers -WebSession $script:session
        Write-Host "Users fetched successfully"
        # Export users to CSV
        $usersResult | Export-Csv -Path "$locUser" -NoTypeInformation
        Write-Host "Users exported to users.csv"
    } catch {
